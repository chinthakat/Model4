...
class TradeLogger:
    ...
    def __init__(self, log_dir: str = "logs/trades", session_name: Optional[str] = None, 
        ...
        # Create session-specific log file with first timestamp
        if not session_name:
            session_name = datetime.now().strftime("trading_session_%Y%m%d_%H%M%S")
        
        # Sanitize session name to ensure it's a valid filename
        ...
        if self.enable_logging:
            self.log_file = self.log_dir / f"{session_name}.csv"
    ...
    def log_trade(self, 
        ...
        self.trade_stats['current_balance'] = net_worth
    ...
    def _save_trade_log(self) -> None:
        ...
        try:
            if len(self.trades_df) > 0:
                # Remove any rows with critical missing data before saving
                ...
                if len(valid_trades) > 0:
                    ...
                    if not self.file_initialized and not self.log_file.exists():
                        ...
                        # Append only the last row to avoid duplicates
                        if len(valid_trades) > 0:
                            last_row = valid_trades.tail(1)
                            if not self.file_initialized:
                                # File exists but we haven't written header in this session
                                last_row.to_csv(self.log_file, index=False, mode='a', header=False)
                                self.file_initialized = True
                            else:
                                # Normal append
                                last_row.to_csv(self.log_file, index=False, mode='a', header=False)
