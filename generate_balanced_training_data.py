...
class SyntheticMarketDataGenerator:
    ...
    def __init__(self, start_price=50000, volatility=0.02):
        self.start_price = start_price
        self.volatility = volatility
        self.timestamp_start = datetime(2024, 1, 1)
        
    def generate_trend_pattern(self, length, trend_strength, noise_level=0.01):
        """Generate a trending pattern (up or down)"""
        trend = np.linspace(0, trend_strength, length)
        noise = np.random.normal(0, noise_level, length)
        return trend + noise
    
    def generate_sideways_pattern(self, length, noise_level=0.005):
        """Generate sideways/ranging pattern"""
        return np.random.normal(0, noise_level, length)
    
    def generate_reversal_pattern(self, length, peak_position=0.5, amplitude=0.05):
    ...
    def create_ohlcv_from_returns(self, returns, base_price, volume_base=1000000):
        ...
        for ret in returns:
            ...
            capped_ret = np.clip(ret, -0.10, 0.10)  # Cap at ±10% per period
            prices.append(prices[-1] * (1 + capped_ret))
        ...
        for i in range(len(prices) - 1):
            ...
            volatility = np.clip(volatility, 0.001, 0.05)  # Cap volatility
            high_price = max(open_price, close_price) * (1 + volatility * np.random.uniform(0, 1))
            low_price = min(open_price, close_price) * (1 - volatility * np.random.uniform(0, 1))
    ...
    def generate_balanced_training_data(self, total_periods=10000):
        ...
        for i in range(patterns_per_type):
            ...
            trend_strength = np.random.uniform(0.08, 0.15)  # 8-15% gain over period
            pattern = self.generate_trend_pattern(segment_length, trend_strength, noise_level=0.005)
            ...
            for cp in correction_points:
                if cp < segment_length - 5:
                    correction = np.random.uniform(-0.02, -0.005)  # 0.5-2% correction
        ...
        for i in range(patterns_per_type):
            ...
            trend_strength = np.random.uniform(-0.15, -0.08)  # 8-15% decline over period
            pattern = self.generate_trend_pattern(segment_length, trend_strength, noise_level=0.005)
            ...
            for bp in bounce_points:
                if bp < segment_length - 5:
                    bounce = np.random.uniform(0.005, 0.02)  # 0.5-2% bounce
            ...
            if pattern_type == 'sideways':
                ...
                for bp in breakout_points:
                    if bp < segment_length - 10:
                        direction = np.random.choice([-1, 1])
                        # Initial breakout
                        pattern[bp:bp+3] += direction * 0.015
                        # Then reversal
                        pattern[bp+3:bp+8] -= direction * 0.018
...
def main():
    ...
    generator = SyntheticMarketDataGenerator(start_price=50000, volatility=0.02)
